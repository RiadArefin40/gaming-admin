...
const fetchNotifications = async () => {
  try {
    const { data, error } = await useFetch("https://api-v2.bajiraj.cloud/deposit/admin/notifications?unread=true", {
      method: "GET",
    });
...
const markAsRead = async (notif) => {
  try {
    const { error } = await useFetch(`https://api-v2.bajiraj.cloud/deposit/admin/notifications/${notif.id}/read`, {
      method: "PATCH",
    });
