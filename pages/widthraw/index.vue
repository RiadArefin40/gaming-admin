...
async function fetchWithdrawals() {
  const res = await fetch("https://api-v2.bajiraj.cloud/withdrawals");
...
async function confirmAction() {
  ...
  const res = await fetch(`https://api-v2.bajiraj.cloud/withdrawals/${selectedWithdrawal.value.id}/${action}`, {
    method: "PATCH",
  });
...
const markWithdrawAsRead = async (notif) => {
  try {
    await useFetch(`https://api-v2.bajiraj.cloud/withdrawals/admin/withdraw_notifications/${notif}/read`, {
      method: "PATCH",
    });
