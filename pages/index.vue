...
const fetchData = async () => {
  ...
  try {
    const [depRes, witRes] = await Promise.all([
      fetch("https://api-v2.bajiraj.cloud/deposit"),
      fetch("https://api-v2.bajiraj.cloud/withdrawals")
    ])
...
async function fetchDepositActions(depositId) {
  try {
    const res = await fetch(`https://api-v2.bajiraj.cloud/deposit/${depositId}/actions`);
...
const fetchWithdrawNotifications = async () => {
  ...
  if (userId){
      try {
    const { data, error } = await useFetch(`https://api-v2.bajiraj.cloud/users/${userId}/balance`, {
      method: "GET",
    });
