...
async function confirmAction() {
  ...
  try {
    ...
    const res = await fetch(`https://api-v2.bajiraj.cloud/deposit/${depositId}/${action}`, {
      method: "PATCH",
...
async function fetchTransactions() {
  loading.value = true;
  try {
    const res = await fetch("https://api-v2.bajiraj.cloud/deposit"); // Replace with real endpoint
...
const markAsRead = async (notif) => {
  try {
    const { error } = await useFetch(`https://api-v2.bajiraj.cloud/deposit/admin/notifications/${notif}/read`, {
      method: "PATCH",
    });
...
const fetchWithdrawNotifications = async () => {
  try {
    const { data, error } = await useFetch("https://api-v2.bajiraj.cloud/withdrawals/admin/withdraw_notifications?unread=true", {
      method: "GET",
...
const markWithdrawAsRead = async (notif) => {
  try {
    await useFetch(`https://api-v2.bajiraj.cloud/withdrawals/admin/withdraw_notifications/${notif.id}/read`, {
      method: "PATCH",
    });
