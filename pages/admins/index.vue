    ...
    <v-dialog v-model="dialogCreate" max-width="600">
      <v-card class="pa-6 rounded-2xl elevation-5">
        ...
        <v-card-text class="d-flex flex-column gap-4">
          <v-text-field label="Username" v-model="createForm.name" outlined />
...
async function updateUser() {
  ...
  try {
    const res = await fetch(
      `https://api-v2.bajiraj.cloud/users/${activeUser.value.id}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
...
async function deleteUser() {
  ...
  try {
    await fetch(`https://api-v2.bajiraj.cloud/users/${activeUser.value.id}`, {
      method: "DELETE",
    });
...
async function toggleUserStatus(user) {
  try {
    const res = await fetch(
      `https://api-v2.bajiraj.cloud/users/${user.id}/toggle-status`,
      { method: "PATCH" }
    );
...
//   try {
//     const res = await fetch(`https://api-v2.bajiraj.cloud/users/${userId}/transactions`);
...
async function fetchBettings(userId) {
  ...
  try {
    const res = await fetch(`https://api-v2.bajiraj.cloud/users/${userId}/bettings`);
...
async function fetchTransactions(userId) {
  ...
  try {
    const [depositRes, withdrawRes] = await Promise.all([
      fetch(`https://api-v2.bajiraj.cloud/deposit/${userId}`),
      fetch(`https://api-v2.bajiraj.cloud/withdrawals/${userId}`)
    ])
