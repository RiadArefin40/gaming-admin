...
const loadSetting = async () => {
  const res = await axios.get("https://api-v2.bajiraj.cloud/payment-gateways/auto-payment");
  enabled.value = res.data.auto_payment_enabled;

    const res1 = await axios.get("https://api-v2.bajiraj.cloud/payment-gateways/widthraw");
  enabledCashout.value = res1.data.widthraw;
};

const toggle = async () => {
  try {
    await axios.post("https://api-v2.bajiraj.cloud/payment-gateways/auto-payment", {
...
const toggleCashout = async () => {
  try {
    await axios.post("https://api-v2.bajiraj.cloud/payment-gateways/widthraw", {
...
const fetchGateways = async () => {
  const { data } = await axios.get("https://api-v2.bajiraj.cloud/payment-gateways/");
  gateways.value = data;
};
...
async function addGateway() {
  await axios.post("https://api-v2.bajiraj.cloud/payment-gateways", gatewayForm.value);
...
async function updateGateway() {
  await axios.put(
    `https://api-v2.bajiraj.cloud/payment-gateways/${activeGateway.value.id}`,
    activeGateway.value
  );
...
async function deleteGateway(gateway) {
  await axios.delete(`https://api-v2.bajiraj.cloud/payment-gateways/${gateway.id}`);
  fetchGateways();
}
