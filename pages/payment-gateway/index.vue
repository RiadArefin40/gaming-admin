...
const loadSetting = async () => {
  const res = await axios.get("https://api.spcwin.info/payment-gateways/auto-payment");
  enabled.value = res.data.auto_payment_enabled;

    const res1 = await axios.get("https://api.spcwin.info/payment-gateways/widthraw");
  enabledCashout.value = res1.data.widthraw;
};

const toggle = async () => {
  try {
    await axios.post("https://api.spcwin.info/payment-gateways/auto-payment", {
...
const toggleCashout = async () => {
  try {
    await axios.post("https://api.spcwin.info/payment-gateways/widthraw", {
...
const fetchGateways = async () => {
  const { data } = await axios.get("https://api.spcwin.info/payment-gateways/");
  gateways.value = data;
};
...
async function addGateway() {
  await axios.post("https://api.spcwin.info/payment-gateways", gatewayForm.value);
...
async function updateGateway() {
  await axios.put(
    `https://api.spcwin.info/payment-gateways/${activeGateway.value.id}`,
    activeGateway.value
  );
...
async function deleteGateway(gateway) {
  await axios.delete(`https://api.spcwin.info/payment-gateways/${gateway.id}`);
  fetchGateways();
}
