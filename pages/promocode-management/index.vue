...
async function fetchPromos() {
  const res = await fetch("https://api.spcwin.info/promos");
...
async function savePromo() {
  ...
  const url = editPromo.value
    ? `https://api.spcwin.info/promos/${promoForm.value.id}`
    : "https://api.spcwin.info/promos";
...
async function deletePromo(item) {
  await fetch(`https://api.spcwin.info/promos/${item.id}`, {
    method: "DELETE"
  });
...
async function toggleStatus(item) {
  await fetch(`https://api.spcwin.info/promos/${item.id}/toggle`, {
    method: "PATCH"
  });
