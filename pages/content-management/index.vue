<template>
            ...
            <v-card class="pt-2 pb-4">
              ...
              <v-card-text class="pa-0">
                <v-data-table-server
                  ...
                  :search="searching"
                  :loading="loading"
                  @update:options="getContentByPagination"
                >
                  <template v-slot:top>
                    <v-text-field
                      class="pl-3"
                  ...
                  <template v-slot:item.action="{ item }">
                      ...
                      <!-- <v-icon class="me-4"> mdi-image-area </v-icon> -->
                    
                    <nuxt-link :to="`/content-management/entry?id=${item.id}`">
                      <v-icon class="me-4"> mdi-pencil </v-icon></nuxt-link
                    >
                  </template>
                </v-data-table-server>
              </v-card-text>
  ...
  const headers = [
    ...
    { title: "Arabic Content", value: "arabicContent", sortable: false },
  ...
  async function getContentByPagination({ page, itemsPerPage, sortBy }) {
  ...
  async function loadTableData({ page, itemsPerPage, sortBy }) {
    ...
    try {
      loading.value = true;
      const role = await useFetch(`${url}/admin/content-managements?${paginationurl}`, {
        method: "GET",
        headers: {
  ...
  async function deleteRoleData() {
    try {
      const role = await useFetch(`${url}/admin/influencer-videos/${deleteRoleId.value}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
